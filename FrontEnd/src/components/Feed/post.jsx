...
const Post = ({ post }) => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await API.get(`/feeds/getComments?feedId=${post._id}`);
  ...
  const toggleLike = () => {
    ...
    const saveLike = async () => {
      try {
        ...
        const response = await API.post("/feeds/like", {
          feedId: post._id,
          userId: userInfo._id,
        });
    ...
    if (comment.trim()) {
      ...
      const saveComment = async () => {
        try {
          ...
          console.log(post._id, userInfo._id, comment);
          const response = await API.post("/feeds/commentPost", {
            feedId: post._id,
            userId: userInfo._id,
            userName: `${userInfo.firstName} ${userInfo.lastName}`,
            comment,
