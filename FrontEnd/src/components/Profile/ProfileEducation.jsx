import React, { useState } from "react";
import styles from "./css/ProfileEducation.module.css";
import ProfileEducationEdit from "./Edit/ProfileEducationEdit";

const ProfileEducation = ({profileOwner, isProfileOwner}) => {
  const [edit, setEdit] = useState(false);

  const education = profileOwner?.education || [
    { name: "Add your Education Institution", stream: "Add Stream", endDate: null },
    { name: "Add your Education Institution", stream: "Add Stream", endDate: null },
  ];
  
  return (
    <div className={styles.educationCard}>
      <div className={styles.header}>
        <div className={styles.title}>Education</div>
        {isProfileOwner &&
        <svg className={styles.editIcon} onClick={() => setEdit(true)} viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M41.6667 158.333H53.5417L135 76.875L123.125 65L41.6667 146.458V158.333ZM25 175V139.583L146.875 18.125L181.667 53.75L60.4167 175H25ZM128.958 71.0417L123.125 65L135 76.875L128.958 71.0417Z" fill="black"/>
        </svg>}
      </div>

      <div className={styles.content}>
        <div className={styles.timeline}>
          <div className={styles.item}>
            <svg className={styles.icon} viewBox="0 0 515 515" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M450.628 364.792V216.729L257.503 321.875L21.4609 193.125L257.503 64.375L493.544 193.125V364.792H450.628ZM257.503 450.625L107.294 369.083V261.792L257.503 343.333L407.711 261.792V369.083L257.503 450.625Z" fill="black"/>
            </svg>

            <div>
              <div className={styles.degree}>
                {education[0]?.name || "Add your Education Institution"}
              </div>
              <div className={styles.field}>
                {education[0]?.stream || "Add Stream"}
              </div>
            </div>
          </div>

          <div className={styles.line}></div>

          <div className={styles.item}>
          <svg className={styles.icon} viewBox="0 0 515 515" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M450.628 364.792V216.729L257.503 321.875L21.4609 193.125L257.503 64.375L493.544 193.125V364.792H450.628ZM257.503 450.625L107.294 369.083V261.792L257.503 343.333L407.711 261.792V369.083L257.503 450.625Z" fill="black"/>
            </svg>
            <div>
              <div className={styles.degree}>
                {education[1]?.name || "Add your Education Institution"}
              </div>
              <div className={styles.field}>
                {education[1]?.stream || "Add Stream"}
              </div>
            </div>
          </div>
        </div>

        <div className={styles.passingYear}>
          <div className={styles.yearText}>Passing Year</div>
          <div className={styles.year}>
            {education[0]?.endDate ? new Date(profileOwner.education[0].endDate).getFullYear() : "Add Year"}
          </div>
          <div className={styles.year}>
            {education[1]?.endDate ? new Date(profileOwner.education[1].endDate).getFullYear() : "Add Year"}
          </div>
        </div>
      </div>

      <div className={styles.showMore}>
                    Show More 
                    <svg viewBox="0 0 227 227" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M66.2109 94.5835L113.503 141.875L160.794 94.5835" stroke="black" stroke-width="22.67" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
            </div>

      <ProfileEducationEdit isOpen={edit} onClose={() => setEdit(false)} userId={profileOwner?._id}/>
    </div>
  );
};

export default ProfileEducation;
